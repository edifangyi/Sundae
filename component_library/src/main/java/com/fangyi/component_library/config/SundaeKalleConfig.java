...
import com.fangyi.component_library.utils.FileUtils;
 ...
 */
public class SundaeKalleConfig {

    private static SundaeKalleConfig sInstance;

    public static SundaeKalleConfig get() {
        if (sInstance == null)
            synchronized (SundaeKalleConfig.class) {
                if (sInstance == null)
                    sInstance = new SundaeKalleConfig();
            }
        return sInstance;
    }
    ...
    private SundaeKalleConfig() {
        this.mPreferences = BaseApplication.getInstance().getSharedPreferences("Kalle", Context.MODE_PRIVATE);

        this.PATH_APP_ROOT = FileUtils.getAppRootPath(BaseApplication.getInstance()).getAbsolutePath() + File.separator + "Kalle";
